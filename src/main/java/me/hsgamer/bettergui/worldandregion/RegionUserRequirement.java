...
public class RegionUserRequirement extends BaseRequirement<Set<UUID>> {
    ...
    protected RegionUserRequirement(RequirementBuilder.Input input) {
        super(input);
    }
    ...
    private Set<UUID> getUUIDSet(Player player, Map<?, ?> map) {
        if (map.containsKey("region")) {
            ...
            if (world != null) {
                return WorldGuardUtil.getMembers(world, id);
            }
        } else {
    ...
    protected Set<UUID> convert(Object value, UUID uuid) {
        ...
        if (value instanceof Map) {
            ...
            return WorldGuardUtil.getMembers(player.getWorld(), StringReplacerApplier.replace(String.valueOf(value), uuid, this));
        }
    }
